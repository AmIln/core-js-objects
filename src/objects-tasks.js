...
function mergeObjects(objects) {
  ...
  for (let i = 0; i < array.length; i += 1) {
    const entriess = Object.entries(array[i]);
    for (let j = 0; j < entriess.length; j += 1) {
      const key = entriess[j][0];
      const value = entriess[j][1];
      if (newObj[key]) {
        newObj[key] += value;
      } else {
        newObj[key] = value;
...
function makeWord(lettersObject) {
  ...
  for (let i = 0; i < keys.length; i += 1) {
    ...
    lettersObject[key].forEach((position) => {
      arrayText[position] = keys[i];
    });
  }
  return arrayText.toString().replace(/[\s.,%]/g, '');
}
